...
\tkzMeth{triangle}{apollonius\_point()} & [\ref{ssub:apollonius_point}] \\
  \bottomrule
  \end{tabular}
  \end{minipage}
  \egroup
...
\tkzMeth{triangle}{feuerbach\_apollonius\_k181(side, EPS)}  &
  See [\ref{ssub:feuerbach_apollonius_k181}] \\

\tkzMeth{triangle}{feuerbach\_apollonius(side, EPS)}  &
  See [\ref{ssub:feuerbach_apollonius}] \\
...
\subsubsection{Method \tkzMeth{triangle}{apollonius\_point()}}
\label{ssub:apollonius_point}
...
\begin{flushright}
\small
\href{https://mathworld.wolfram.com/ApolloniusPoint.html}{\textit{(Reference: Weisstein, Eric W. "Apollonius Point." MathWorld)}}
\end{flushright}
...
\subsubsection{Method \tkzMeth{triangle}{feuerbach\_apollonius\_k181}(<EPS<)}
\label{ssub:feuerbach_apollonius_k181}

This method constructs a circle through three points obtained from simple
line--line intersections involving:

\begin{itemize}
  \item the three Feuerbach points $E_a,E_b,E_c$,
  \item the Apollonius point $K_{181}$ of the triangle,
  \item the Spieker center~$S$.
\end{itemize}

\texttt{Construction idea: }
For each Feuerbach point $E_a$, consider the lines
$(A,K_{181})$ and $(S,E_a)$.
Their intersection defines a point $x_a$.
Repeating the process with $B,E_b$ and $C,E_c$ gives three points
$x_a,x_b,x_c$ which always lie on a unique circle.

This circle is the \emph{Apollonius--K181 Feuerbach circle}.
...
\texttt{Return: }
A \tkzClass{circle} object: the circumcircle of the three intersection points
$x_a,x_b,x_c$.



\subsubsection{Method \tkzMeth{triangle}{feuerbach\_apollonius}(EPS)}
\label{ssub:feuerbach_apollonius}

This method constructs a remarkable Apollonius-type circle associated with the
triangle.
The construction is based on three Feuerbach points and an inversion in a
circle orthogonal to the excircle touching point.

\texttt{Definition: }
Let $E_a,E_b,E_c$ be the three Feuerbach contact points of the triangle, and
let $J_a$ be the excenter opposite to vertex~$A$.
Let $C_{J_aE_a}$ be the circle with center $J_a$ passing through $E_a$, and
let~$S$ be the Spieker center.

The circle $C_{J_aE_a}$ admits a unique orthogonal circle through $S$.
Inverting the Euler circle in this orthogonal circle maps the three Feuerbach
points to points $x_a,x_b,x_c$ which lie on a common circle.
This is the \emph{Feuerbach--Apollonius circle}.
...
\texttt{Return: }
A \code{circle} object: the circle passing through the three inverted
Feuerbach points.
