\newpage
\section{Global Variables and constants} % (fold)
...
\subsection{Global Variables} % (fold)
...
\subsubsection{Function \tkzFct{tkz-elements}{reset\_defaults()}} % (fold)
...
\subsubsection{Function \tkzFct{tkz}{tkz.set\_nb\_dec(n)}} % (fold)
...
%
% \noindent
% \textbf{Effect:} Updates both \code{tkz\\\_nb\\\_dec} and \code{tkz\\\_epsilon} (which becomes $10^{-n}$).
