...
\tkzMeth{line}{harmonic\_both(r)}  & [\ref{ssub:harmonic_both}; \ref{sub:harmonic_division_with_tkzphi}] \\
\tkzMeth{line}{harmonic(mode, pt)}  & [\ref{ssub:harmonic}] \\


\tkzMeth{line}{report(d,pt)}    &[\ref{ssub:method_report}]\\

\tkzMeth{line}{collinear\_at(pt,k)}  & [ex. \ref{ssub:method_line_collinear__at}]\\
  \midrule
...
\tkzMeth{line}{LPP} & or c\_l\_pp(pt, pt) [\ref{ssub:c_l_pp}]  \\
\tkzMeth{line}{LLP} & or c\_ll\_p(L, pt) [\ref{ssub:method_c__ll__p}]  \\
\tkzMeth{line}{LLL(L, L)} & [\ref{ssub:method_LLL}]  \\
...
\texttt{Purpose: } This method constructs and returns a \tkzname{point} located at a given distance \( r \) from a reference point along the direction of the line.
...
\texttt{Alias: } The synonym \code{point\_at\_distance} is provided for readability, especially when the geometrical intent is to “locate a point at a given distance along a direction”.

\texttt{Returned value :}
...
\subsubsection{Method \tkzMeth{line}{barycenter(ka, kb)}} %(fold)
...
You can obtain the midpoint of a segment using an attribute of the \code{line} object.
...
\subsubsection{Method \tkzMeth{line}{harmonic(mode, arg)}}
\label{ssub:harmonic}

This method provides a unified interface for harmonic division.

\begin{itemize}
  \item \code{mode = "internal"} returns the internal harmonic point.
  \item \code{mode = "external"} returns the external harmonic point.
  \item \code{mode = "both"} returns both harmonic points.
\end{itemize}

The argument \code{arg} is:
\begin{itemize}
  \item a point when \code{mode} is \code{"internal"} or \code{"external"},
  \item a ratio when \code{mode} is \code{"both"}.
\end{itemize}
...
\subsubsection{Method \tkzMeth{line}{collinear\_at(pt,<r>)}} %(fold)
\label{ssub:method_line_collinear__at}

This method returns a point located on a line parallel to $(AB)$ and passing through a given point.
The resulting point is placed at a distance proportional to the length of $AB$, in the same direction.

\medskip

If the second argument \meta{r} is omitted, it defaults to $1$.
In that case, the method produces a segment of the same length and direction as $[AB]$.

\medskip

In addition to the scale-factor form, this method also accepts an option table
\code{\{length =~$\ell$\}}, allowing the distance $CD$ to be specified
\emph{absolutely}, independently of~$AB$.

\medskip

\textbf{Example interpretations:}
\begin{itemize}
  \item If \code{L.AB:collinear\_at(z.C)} produces point $E$, then $CE = AB$ and $(AB) \parallel (CE)$.
  \item If \code{L.AB:collinear\_at(z.C, 0.5)} produces point $D$, then $CD = 0.5 \cdot AB$ and $(AB) \parallel (CD)$.
  \item If \code{L.AB:collinear\_at(z.C, \{length = 2\})} produces point $F$, then $CF = 2$ (in the current unit) and $(AB) \parallel (CF)$. It is also possible to use a negative value.
\end{itemize}
...
By default, if the second argument is omitted, the factor is $1$.
...
\begin{itemize}
  ...
  \item If \code{L.AB:orthogonal\_at(z.C, \{length = 2\})} gives point $F$, then $CF = 2$ (in the current unit) and $(AB) \perp (CF)$. It is also possible to use a negative value.
\end{itemize}
...
\medskip
\texttt{Syntax: } \verb|z.M = L.AB:random()|
...
\medskip
\texttt{Alias: } \code{parallel\_from}


\texttt{Purpose: } This method constructs a new object of type \code{line}, parallel to the original line and passing through the given point.

\medskip

Unlike the \code{collinear\_at} method, which simply returns a point translated along the same direction, \code{ll\_from} returns a full line object. This allows further operations such as intersections, projections, or constructions involving new points along that line.

\medskip

\texttt{Use this method when you need a new line object rather than a point.}.

 The choice between \code{ll\_from} and \code{collinear\_at} depends on the context and on whether you need to work with the resulting direction as a geometric object.
...
\subsubsection{Comparison between \texttt{collinear\_at} and \texttt{ll\_from} methods}

\bgroup
  \small
  \begin{tabular}{@{} l ll @{}}
  \toprule
  \smallbf{Aspect} & \code{collinear\_at(pt, r)} & \code{ll\_from(pt)} \\
  \midrule
...
\texttt{Alias: } \code{orthogonal\_from}


\texttt{Purpose: } This method constructs a new object of type \code{line}, perpendicular to the original line and passing through the given point.
...
\texttt{Use this method when a perpendicular line is needed as a geometric object.}

Choose between \code{orthogonal\_at} and \code{ortho\_from} depending on whether you need a point or a line.
...
\texttt{Alias: } You may also call this method using the alternative names \code{perpendicular\_bisector()} or \code{bisector()}.
...
\subsubsection{Method \tkzMeth{line}{collinear\_at\_distance(d)}}
\label{ssub:method_line_collinear_at_distance}

\texttt{Description: } This method creates a new line parallel to the current one, at a signed distance \texttt{d}.
The sign of \texttt{d} follows the usual convention:
\texttt{d > 0} shifts the line to the \emph{left} of the oriented segment \(\overrightarrow{pa\,pb}\),
and \texttt{d < 0} to the \emph{right}.

\medskip
\texttt{Syntax: } \verb|L' = L:collinear_at_distance(d)|

\medskip
\texttt{Arguments: } \texttt{d} — signed offset distance (real number, same unit as coordinates).

\medskip
\texttt{Return value: }

A new \tkzname{line} object parallel to \texttt{L} at distance \texttt{d}.
...
\texttt{Purpose: }This method is not intended for frequent use, but it can be useful in specific contexts.
...
\subsubsection{Method \tkzMeth{line}{LPP}(p, p)}
\label{ssub:c_l_pp}

This method is presented in the section \tkzname{Apollonius' problem} [\ref{sub:LPP}]


\subsubsection{Method \tkzMeth{line}{LLP}(L, p)}
\label{ssub:method_c__ll__p}

This method is presented in the section \tkzname{Apollonius' problem} [\ref{sub:LLP}]

%====================================================================
\subsubsection{Method \tkzMeth{line}{LLL}(L1,L2[,which])}
%====================================================================
\label{ssub:method_LLL}

This method is presented in the section \tkzname{Apollonius' problem} [\ref{sub:LLL}]
...
Given three points, let's look at how, using the \tkzname{path} method, we can draw the triangle defined by these three points. In this case, only the two ends of each side are required:
