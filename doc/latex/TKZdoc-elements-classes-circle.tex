...
The \tkzVar{circle}{C} variable is a table reserved for storing circle objects. Although its use is optional and any valid variable name may be used (e.g., \code{Circles}), it is strongly recommended to adopt the standard name \texttt{C} to ensure consistency and readability. If a custom variable is used, it must be initialized manually. The \tkzFct{tkz-elements}{init\_elements()} function will reset the \texttt{circle}{C} table if it has already been defined.
...
\bgroup
  ...
  \tkzMeth{circle}{is\_tangent(L)} & [\ref{ssub:method_circle_is_tangent}]  \\
  \tkzMeth{circle}{is\_secant(L)} & [\ref{ssub:method_circle_is_secant}]  \\
  \tkzMeth{circle}{is\_disjoint(L)} & [\ref{ssub:method_circle_is_disjoint}]  \\
  \tkzMeth{circle}{line\_position(L)} & [\ref{ssub:method_circle_line_position}]\\
  \tkzMeth{circle}{lines\_position(L)} & [\ref{ssub:method_circle_lines_position}]\\
  ...
  \tkzMeth{circle}{inversion\_neg(obj)} & [\ref{ssub:inversion_neg}]\\
  ...
  \tkzMeth{circle}{similitude(mode, C)} &  [\ref{ssub:similitude}] \\

  \tkzMeth{circle}{radical\_center(C1<,C2>)} & [\ref{ssub:radical_center} ]  \\
 \tkzMeth{circle}{pole(L)} & [\ref{ssub:method_circle_pole}]\\
 \midrule
   \textbf{Lines}  & \\
  \midrule
...
\newpage
  ...
  \tkzMeth{circle}{CPP(pt,pt)} & [\ref{ssub:method_c__c__pp}] \\
  \tkzMeth{circle}{CCP(C,pt)} &[\ref{ssub:method_c_cc_p}]  \\
  \tkzMeth{circle}{CLP(L,pt,<'inside'>)} & [\ref{ssub:method_c_lc_p}]  \\
  \tkzMeth{circle}{CLL}& \ref{ssub:method_c_cll}\\
  \tkzMeth{circle}{CCL}(C2, L) & \ref{sub:CCL}\\
  \tkzMeth{circle}{CCC(C2, C3[, opts])} &  \ref{sub:CCC}\\
  \tkzMeth{circle}{CCC\_gergonne}(C2, C3) &  \ref{sub:CCC_gergonne}\\
...
\textbf{1. Function \code{through(center, radius, <angle>])}}
...
\textbf{2. Function \code{diameter(A, B, <'swap'> or <angle>)}}
...
\label{ssub:method_circle_is_tangent}
...
\subsubsection{Method \tkzMeth{circle}{is\_secant(L)}}
\label{ssub:method_circle_is_secant}
see the example above

\subsubsection{Method \tkzMeth{circle}{is\_disjoint(L)}}
\label{ssub:method_circle_is_disjoint}
see the example above
...
These two methods determine the position of a point relative to a circle or the closed disk it defines. Si nécessaire, pour toutes les fonctions suivantes, vous pouvez ajuster la précision pour déterminer les résultats à l'aide d'un argument optionnel. Par défaut cette précision est liée à la variable globale \verb|tkz.epsilon = 1e-8|
...
\textbf{Differences between the two methods:}
...
\subsubsection{Method \tkzMeth{circle}{line\_position(L)}}
\label{ssub:method_circle_line_position}

\texttt{Purpose: }

This method classifies the relative position of a line with respect to a circle.
It returns whether the line is \texttt{disjoint}, \texttt{tangent} or \texttt{secant}.

\medskip
\texttt{Syntax: } \verb|pos = C.AB:line_position(L.CD)|

\medskip
\texttt{Arguments: }

 \texttt{L} — a \tkzname{line} object to be tested against the circle.
...
\begin{itemize}
  \item \texttt{"disjoint"} — no intersection between line and circle,
  \item \texttt{"tangent"} — line tangent to the circle (one point in common),
  \item \texttt{"secant"} — line intersecting the circle at two points.
\end{itemize}
...
\begin{itemize}
  \item The radius is computed from the center and the defining point (\texttt{self.through})
        to ensure numerical stability.
  \item The tolerance \tkzVar{tkz}{tkz.epsilon} is used to handle rounding errors near tangency.
\end{itemize}
...
\subsubsection{Method \tkzMeth{circle}{lines\_position(L1, L2, mode)}}
\label{ssub:method_circle_lines_position}

\texttt{Purpose: }

This method determines the relative position of a circle with respect to two lines.
It analyses which angular or strip sectors are touched by the circle depending on whether the lines are parallel or secant.
The result is returned as a list of integers corresponding to the sectors intersected by the circle.

\texttt{Syntax: } \verb|sectors = C:lines_position(L1, L2 [, mode])|

\texttt{Arguments: }

\begin{itemize}
  \item \texttt{L1}, \texttt{L2} — two \tkzname{line} objects.
  \item \texttt{mode} — an optional string:
    \begin{itemize}
      \item \texttt{"parallel"} — if the lines are known to be parallel;
      \item any other value (or omitted) — default, for secant lines.
    \end{itemize}
\end{itemize}
...
For secant lines:
\begin{itemize}
  \item sectors are numbered counterclockwise from the first line to the second:
        \texttt{1, 2, 3, 4};
  \item for example, \texttt{\{1\}} means the circle is entirely inside the first sector,
        and \texttt{\{4, 1, 2\}} means the circle touches three adjacent sectors.
\end{itemize}

For parallel lines:
\begin{itemize}
  \item sectors are numbered as:
        \texttt{1} – between the lines,
        \texttt{2} – outside on the side of \texttt{L1},
        \texttt{3} – outside on the side of \texttt{L2};
  \item examples:
        \texttt{\{1\}} → circle entirely between the lines;
        \texttt{\{1,2\}} → tangent to \texttt{L1};
        \texttt{\{1,2,3\}} → circle large enough to cross both lines.
\end{itemize}

\texttt{Remarks: }

\begin{itemize}
  \item The classification is purely geometric and does not compute intersection points.
  \item The tolerance \tkzVar{tkz}{tkz.epsilon} is used to handle numerical approximations in tangency detection.
\end{itemize}

\texttt{Example (secant lines)}

\begin{tkzexample}[latex=.48\textwidth]
\directlua{
  z.A = point(0, 0)
  z.B = point(3, -2)
  L.AB  = line (z.A, z.B)
...
\subsubsection{Method \tkzMeth{circle}{similitude(C,mode)}}
\label{ssub:similitude}

This method unifies the two similitude-center functions.

\begin{itemize}
  \item \code{mode = "internal"} returns the internal center of similitude.
  \item \code{mode = "external"} returns the external center of similitude.
\end{itemize}

\medskip
\noindent\texttt{Example: }
\begin{verbatim}
...
\directlua{
   ...
   z.x2, z.y2 = L.rAC:get()
   z.x3, z.y3 = L.rBC:get()
   _,z.t = C.Aa:radical_circle(C.Bb,C.Cc).through}
\begin{center}
\begin{tikzpicture}
   \tkzGetNodes
   ...
   \tkzDrawPoints(A,B,C,w)
   \tkzLabelPoints[below right](A,B,C,w)
\end{tikzpicture}
\end{center}
...
\subsubsection{Method \tkzMeth{circle}{pole(L)}}
\label{ssub:method_circle_pole}
...
\texttt{Purpose:}
Compute the \emph{pole} of a line \(L\) with respect to the current circle.
The pole is the point whose polar is exactly the line \(L\).
...
\texttt{Returns:}
\begin{itemize}
  \item the point \(P\) such that the polar of \(P\) w.r.t.\ the circle is \(L\);
  \item or \code{nil,"INFINITE"} if the line passes through the circle’s center.
\end{itemize}

\medskip
\texttt{Geometric principle:}
Let \(O\) be the center of the circle and \(R\) its radius.
Denote by \(H\) the orthogonal projection of \(O\) onto the line \(L\).
If \(H \neq O\), the inversion of \(H\) in the circle,
satisfying \(OH \cdot OP = R^2\),
is the pole \(P\) of line \(L\).

\[
P = \mathrm{Inv}_{(O,R)}(H),
\qquad \text{with } H = \mathrm{proj}_L(O)
\]

If \(H = O\) (i.e.\ the line passes through the center),
the pole is an \emph{ideal point} on the perpendicular to \(L\) through \(O\).
...
\begin{center}
  ...
  \tkzDrawSegments(M,M' A,D B,D A,B C,D
    T,C T',C)
  ...
  \tkzMarkAngles[mark=|,size=.75](T,C,M C,T,T'
       C,D,T T,D,M)
  \tkzMarkAngles[mark=||,size=.75](M',C,T'
  T,T',C T',D,C M',D,T')
\end{tikzpicture}
\end{center}
\end{minipage}
...
\texttt{Radical circle of three given circles.}
...
\subsubsection{Method \tkzMeth{circle}{CPP}}
\label{ssub:method_c__c__pp}

This method is presented in the section \tkzname{Apollonius' problem} [\ref{sub:CCP}]

\subsubsection{Method \tkzMeth{circle}{CCP(C,p[,mode])}}
\label{ssub:method_c_cc_p}

This method is presented in the section \tkzname{Apollonius' problem} [\ref{sub:CCP}]


\subsubsection{Method \tkzMeth{circle}{CLP}(L,p)}
\label{ssub:method_c_lc_p}

This method is presented in the section \tkzname{Apollonius' problem} [\ref{sub:CLP}]



\subsubsection{Method \tkzMeth{circle}{CLL}(L,L,<choice,inside>)}
\label{ssub:method_c_cll}

This method is presented in the section \tkzname{Apollonius' problem} [\ref{sub:CLL}]
...
\textbf{Midcircle between a circle and a line}
...
\begin{enumerate}[label=(\roman*)]
  \item \texttt{The circle and the line intersect};
  \item \texttt{The circle and the line are disjoint};
  \item \texttt{The circle and the line are tangent}.
\end{enumerate}

Let's look at each case:

\texttt{Note: } It should be noted that in each case, inversion allows us to verify that the two objects are inverses of each other.
...
\subsubsection{Method \tkzMeth{circle}{inversion\_neg(obj)}}
\label{ssub:inversion_neg}


\texttt{Syntax: }

\verb|local Q = C:inversion_neg(obj)|

\texttt{Purpose: }

This method performs the inversion with respect to the circle \(C\) but with a
\emph{negative power}.
For a point \(P \neq O\), this means that
\[
\overrightarrow{OP'} \cdot \overrightarrow{OP} = -R^2,
\]
that is, \(P'\) is the inverse of \(P\) on the opposite half–line of \(OP\).
From a geometric point of view, the image set (line or circle) is the same as
with the ordinary inversion, but the points are taken with reversed orientation
along that set.

\texttt{Arguments: }
\begin{itemize}
  \item \verb|obj| – a \tkzname{point}, a \tkzname{line} (possibly not passing through \(O\)),
  or a \tkzname{circle}.
\end{itemize}

\texttt{Returns: }
\begin{itemize}
  \item If \verb|obj| is a point \(P\neq O\): the point \(P'=\texttt{inversion\_neg}(P)\).
  \item If \verb|obj| is a line not passing through \(O\): the same circle image
  as with \verb|C:inversion(line)| (it passes through \(O\)), but the orientation of the
  points on it is reversed.
  \item If \verb|obj| is a line passing through \(O\): the image is the same
  line.
  \item If \verb|obj| is a circle not passing through \(O\): the image is the same
  circle as with \verb|inversion|, but with reversed orientation.
  \item If \verb|obj| is a circle passing through \(O\): the image is the same line.
\end{itemize}
...
\]
This is equivalent to performing the ordinary inversion followed by a
central symmetry with respect to \(O\) (taking the antipode).

For lines and circles, \verb|inversion_neg| produces the same geometric image as
\verb|inversion|; the negative sign only affects the parametrization along that
image.

\texttt{About the next example: }

The aim is to find the circles passing through a point $P$ tangent to a line $L$ and a circle.


In the example given above, the point
\verb|P2 = C.inv2:inversion_neg(P)| lies on the required circle because
the negative inversion places the image of \(P\) on the same geometric locus as
the standard inversion, but on the opposite direction relative to \(O\).
When combined with \verb|CPP(P,P2)|, this choice selects the correct circle among
the possible tangent ones, ensuring that \(P_2\) lies exactly on the desired
solution circle.
...
\subsubsection{Method \tkzMeth{circle}{path(p1, p2, N)}}
\label{ssub:circle_path}

\texttt{Purpose: } The \tkzClass{circle} class includes a method \code{path} to create a \tkzClass{path} object representing a circular arc between two points on the circle.
